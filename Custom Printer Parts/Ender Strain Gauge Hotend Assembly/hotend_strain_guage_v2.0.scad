...
module switch_cut_out() {
  ...
  // Switch Mount Holes
  translate([(casing_wall_thickness / 2.00) + (d2f_length / 2.0) - (m2_hole_offset / 2.0) + casing_side_extension, (d2f_width / 2.00) + 2.00, 0.0]) {
    rotate([0, 0, 90]) {
      translate([0.0, d2f_hole_position_from_centre, 0.0]) {
        cylinder(d=m2_hole_diameter + hole_tight_tolerance + extra_tolerance_for_switch_holes + extra_tolerance_for_switch_holes, h=switch_hole_hight);
      }
...
module StrainGuage() {
  difference() {
    ...
    translate([0.0, casing_top_buffer, 0.0]) {
      ...
      translate([casing_wall_thickness + 0.80 + casing_side_extension, total_casing_height / 2.0 + d2f_pins - 1.00, (total_casing_depth / 2.0)]) {
      ...
      translate([casing_side_extension, - (total_casing_height) + casing_top_buffer + 2.5 - 0.50, 0.0]) {
        // Mounting holes with countersinking - horizontal, through-holes, properly positioned
